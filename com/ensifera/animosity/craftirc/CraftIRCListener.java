...
public class CraftIRCListener extends PlayerListener {
    ...
    public boolean onCommand(CommandSender sender, Command command, String commandLabel, String[] args) {
        ...
        boolean permCheck = ((sender instanceof Player) || (sender instanceof ConsoleCommandSender)) ;

        if (commandName.equals("irc")) {
            if (permCheck && !this.plugin.checkPerms((Player) sender, "craftirc.irc")) {
                if (this.plugin.isDebug()) CraftIRC.log.info(String.format(CraftIRC.NAME + " CraftIRCListener onCommand(): commandName=irc"));
                return true;
            }
            return this.cmdMsgToAll(sender, args);
        } else if (commandName.equals("ircm")) {
            
            if (permCheck && !this.plugin.checkPerms((Player) sender, "craftirc.ircm")) {
                if (this.plugin.isDebug()) CraftIRC.log.info(String.format(CraftIRC.NAME + " CraftIRCListener onCommand(): commandName=ircm"));
                return true;
            }
            return this.cmdMsgToTag(sender, args);
        } else if (commandName.equals("ircwho")) {
            if (permCheck && !this.plugin.checkPerms((Player) sender, "craftirc.ircwho")) {
                if (this.plugin.isDebug()) CraftIRC.log.info(String.format(CraftIRC.NAME + " CraftIRCListener onCommand(): commandName=ircwho"));
                return true;
            }
            return this.cmdGetIrcUserList(sender, args);
        } else if (commandName.equals("admins!")) {
            if (permCheck && !this.plugin.checkPerms((Player) sender, "craftirc.admins!")) {
                if (this.plugin.isDebug()) CraftIRC.log.info(String.format(CraftIRC.NAME + " CraftIRCListener onCommand(): commandName=admins!"));
                return true;
            }
            return this.cmdNotifyIrcAdmins(sender, args);
        } else if (commandName.equals("ircraw")) {
            if (permCheck && !this.plugin.checkPerms((Player) sender, "craftirc.ircraw")) {
                if (this.plugin.isDebug()) CraftIRC.log.info(String.format(CraftIRC.NAME + " CraftIRCListener onCommand(): commandName=ircraw"));
                return true;
            }
    ...
    private boolean cmdNotifyIrcAdmins(CommandSender sender, String[] args) {
        try {
            if (args.length == 0 || !(sender instanceof Player))
                return false;
            this.plugin.noticeAdmins("[Admin notice from " + ((Player) sender).getName() + "] "
